...
from test_mattermost_ingress import (
    ...
    ROUND16_RESIDUAL_REPRESENTATIVES,
    ROUND17_REPRESENTATIVES,
    _round17_namespace,
)
...
def test_round16_residual_sources_are_malformed_in_direct_channel(tmp_path, letter, source):
    service, rest, conversation, clinical = clinical_ingress(tmp_path)
...
def test_round17_post_normalization_sources_are_malformed_in_direct_channel(tmp_path, meaning, source):
    service, rest, conversation, clinical = clinical_ingress(tmp_path)
    message = f"@restricted-bot {_round17_namespace(source, meaning)} {PATIENT}"
    candidate = post(message=message)
    rest.posts[ROOT] = candidate
    service.handle(event(candidate, channel_type="D"))
    assert mattermost_ingress._clinical_command(message, "restricted-bot") == ("malformed", None)
...
def test_clinical_delivery_timeout_after_claim_becomes_ambiguous_without_reauthorization_retry(tmp_path):
    service, rest, conversation, clinical = clinical_ingress(tmp_path)
    source = rest.posts[ROOT]
    record_tag = service.outbox.record_tag(service._clinical_envelope(source, PATIENT))
    calls = []
    ...
    assert durable.reason == "stale_in_flight" and durable.envelope is None
    assert len(calls) == 1
    assert conversation.calls == [] and rest.created == []

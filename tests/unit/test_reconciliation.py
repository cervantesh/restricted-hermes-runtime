...
class Gateway:
    ...
    def status(self, *_): return self.state
    def fence(self, *_): self.fenced=True; return self.state
...
def test_dispatch_started_becomes_indeterminate_and_never_calls_inference():
    ...
    assert Reconciler(content, gateway).reconcile_expired(tenant_id="t", turn_id="x", owner="r") is TurnState.INDETERMINATE
...
def test_reserved_is_fenced_before_failed():
    ...
    assert Reconciler(content, gateway).reconcile_expired(tenant_id="t", turn_id="x", owner="r") is TurnState.FAILED

...
def test_migration_secret_is_pinned_and_migration_has_only_native_cloud_sql_container():
    ...
    assert source.count('"--quitquitquit"') == 2
    assert source.count('path = "/readiness"') == 2
    assert source.count('"--exit-zero-on-sigterm"') == 2
    assert source.count('depends_on = ["cloud-sql-proxy"]') == 2
...
def test_connectors_have_explicit_supported_capacity_and_psc_uses_the_reserved_address_uri():
    ...
    assert "replace_triggered_by = [google_compute_address.vertex_psc]" in main
    assert "self.address == google_compute_address.vertex_psc.address" in main
    assert "Vertex regional endpoint address must match the reserved PSC address." in main

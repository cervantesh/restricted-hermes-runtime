...
def test_current_egress_witness_uses_real_red_green_and_content_safe_receipt_boundary():
    ...
    for phrase in ("network", "connect", "disconnect", "controlled RED", "controlled external", "status", "cleanup", "--ipv6", "diagnostic"):
        assert phrase in runner
    for phrase in ("canonical_bytes", "candidate_receipt", "controlled_red", "controlled_external", "network_membership", "raw endpoints"):

...
def restricted_container_control_evidence() -> dict[str, object]:
    ...
    identity_probe = "import json,os; print(json.dumps({'uid':os.getuid(),'gid':os.getgid(),'groups':sorted(os.getgroups())}))"
    for service, expected_identity in expected.items():
        ...
        identity = json.loads(compose("exec", "--no-TTY", service, "python", "-c", identity_probe).stdout)

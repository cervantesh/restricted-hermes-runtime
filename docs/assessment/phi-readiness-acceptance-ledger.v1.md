# Restricted clinical readiness: six-lane acceptance ledger v1

## Frozen frame and scope

**Contract version:** `restricted-clinical-readiness-ledger.v1`.
**Inspected runtime base:** `4a818050830e84ebc55b926bf1ee71c0945792de`.
**Evidence status at creation:** no tests or external controls were executed by
this documentation task. Existing tests below are inspected entrypoints, not
new PASS evidence. The commit containing this document freezes its acceptance
contract; it does not freeze a deployment or establish readiness.

**Base-update impact:** this ledger was rebased from inspected base
`a57d4e89301556b8f5850a4a89e1b6a72059b5db`. The intervening commit restores
the default clinical-staging source-build Compose overlay and adds its tests and
documentation. It changes no predicate, schema, candidate identity, retained
witness, or external observation in this ledger. No row gains PASS evidence or
changes from MISSING/BLOCKED because of that base update.
...
- **FROZEN:** the observable requirement and artifact schema are fixed by v1.
...
The existing `tools/verify_assessment_bundle.py` at the inspected base is the
schema authority for profile `restricted-clinical-candidate.v1`, manifest
`restricted-clinical-assessment-bundle.v1`, and the nine payload paths below.
...
|---|---|---|---|
| A0 | This `phi-readiness-acceptance-ledger.v1.md`; documentary schema above | orchestrator -> all six lanes | ledger version + commit; none; row review and documentation commit |
...
| A9 | `independent-review/findings.json`; bundle profile including reviewer/authority/conflict statement, findings and technical GO/NO_GO | independent evaluator -> final bundle builder/operator | K + A10P manifest hash; A1-A8,A10P; I5 freezes the independent disposition of the exact pre-review input |
| A10P | `bundles/pre-review/assessment.manifest.json`, fixed `README.md`, exact `verify_assessment_bundle.py`; existing bundle schema | pre-review bundle builder -> independent evaluator | K + canonical pre-review manifest and every file hash; A1-A8; V5P freezes the immutable review input with the schema-required A9 path truthfully NOT_VERIFIED |
| A10F | `bundles/final/assessment.manifest.json`, fixed `README.md`, exact `verify_assessment_bundle.py`; existing bundle schema | final bundle builder -> clean-room verifier/operator | K + canonical final manifest and every file hash; A1-A9,A10P; V5F freezes a new immutable bundle after I5 without replacing A10P |
...
| V5F | Insert findings that do not bind A10P, or overwrite the pre-review bundle while adding I5 | Final bundle is a distinct immutable artifact; exact A9 binds A10P and invalid substitution remains NOT_READY | T1::test_profile_rejects_a_dummy_or_incomplete_candidate_from_ready; `manual:final-bundle-input-binding-v1` | Py+Human; MISSING | A10F; V5P,I5 |
...
| I5 | Evaluator identifies technical failure, missing witness or conflict of interest | Attributable exact-K NO_GO or bounded GO binds the A10P manifest; no unresolved required control silently waived | T1::test_pass_and_fail_require_complete_proof_shapes_and_no_go_cannot_be_ready; `manual:independent-technical-disposition-v1` | Py+independent evaluator; BLOCKED | A9; I4 |
...
record can bind the pre-review input hash while the final manifest binds that
record, avoiding a cyclic or self-referential hash.
...
2. A predicate/schema change requires `v2` (or the corresponding schema's next

"""Cold backup-bundle codec with explicit, injected staging dependencies.

This module is introduced as an isolated cold-recovery dependency.  Nothing in
`clinical_staging.py` imports it yet: the lifecycle integration is a separate,
separately proven slice.

Every staging fact the codec needs arrives through `BackupContract` rather than
an import, so the codec cannot silently re-introduce an older staging contract.
Two fields exist specifically to keep the *current* admitted-subject line
authoritative:

* `marker_keys` is the exact key set of the current staging marker.  The
  historical cold-recovery code hard-coded an earlier key set that predates
  `image_mode` and `subject_admission`; a bundle validated against that older
  shape would silently drop the subject-admission record.
* `validate_marker` is the caller's own marker invariant (subject admission,
  sealed environment).  The codec applies it to the *archived* marker, so a
  cold bundle cannot re-enter the lifecycle carrying a marker that the live
...
class BackupContract:
    ...
    validate_marker: Callable[[Mapping[str, Any]], None]
...
def require_sha256(contract: BackupContract, value: Any, *, name: str) -> str:
...
def safe_archive_name(contract: BackupContract, name: str) -> str:
    ...
    try:
        with tarfile.open(path, "r:") as archive:
            ...
            for member in archive.getmembers():
                name = safe_archive_name(contract, member.name)
                if not name:
                    if member.isdir():
                        continue
...
def build_backup_manifest(
    ...
    """Create the content-safe manifest.

    The manifest carries no secret material.  The subject-admission record is
    deliberately *not* copied here: it stays inside the private state archive,
    whose content digest and ownership digest this manifest binds.
    """
...
def write_backup_manifest(
    ...
    """Publish the manifest with an exclusive create that leaves no residue.

    The staging module's general-purpose atomic writer keeps a durable
    `.<name>.atomic.lock` sibling.  That is correct for a long-lived private
    state directory, but in a published bundle it would become an unexpected
    member and fail the bundle's own exact-member check.  A backup directory is
    created once, under the operator boundary, so an exclusive create plus
    fsync is both sufficient and exact.
    """
    path = backup_dir / contract.manifest_name
    ...
    try:
        ...
        raise contract.error_type("could not publish the backup manifest") from exc
...
def read_backup_manifest(contract: BackupContract, backup_dir: Path) -> dict[str, Any]:
    try:
        value = json.loads(
...
def _validate_backup_manifest_shape(
    ...
    *,
    ...
    for name, value in members.items():
        ...
        require_sha256(contract, value["sha256"], name=f"backup member hash for {name}")
        require_sha256(
            contract,
    ...
    try:
        with tarfile.open(path, "r:") as archive:
            matching = [
                member
                for member in archive.getmembers()
                if safe_archive_name(contract, member.name) == name
            ]
...
def validate_archived_marker(
    ...
    *,
    ...
    if set(marker) != set(contract.marker_keys):
    ...
    # The live admission and sealed-environment invariants stay authoritative: a
    # cold bundle may not re-enter the lifecycle carrying a marker that the
    # current source would have refused.
...
def validate_backup_bundle(
    ...
    require_sha256(contract, expected_manifest_sha256, name="external manifest hash")
    ...
    manifest = _validate_backup_manifest_shape(
        contract,
        read_backup_manifest(contract, backup_dir),
        project=project,
        state_dir=state_dir,
    )
...
def materialize_backup_snapshot(
    ...
    *,
    ...
    try:
        ...
        (snapshot / contract.volume_directory).mkdir(mode=0o700)
